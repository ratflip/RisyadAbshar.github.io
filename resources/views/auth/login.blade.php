...
<body class="min-h-screen bg-brand-cream flex items-center justify-center p-4 font-public-sans">
    <div class="w-full max-w-[420px] bg-white rounded-2xl p-10 shadow-[0_10px_30px_rgba(139,69,19,0.1)] border border-brand-brown/5">
        <div class="text-center mb-8">
            ...
            <p class="text-sm text-brand-brown/70 mt-2">Silakan masuk untuk melanjutkan pesanan Anda</p>
        </div>

        @if(session('success'))
            <div class="mb-4 p-3 bg-green-100 text-green-800 border border-green-200 text-sm rounded-lg text-center font-medium">
                {{ session('success') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('login.post') }}" class="space-y-5">
            ...
            <button type="submit" 
                    class="w-full bg-brand-green text-white py-3 rounded-xl font-semibold text-sm hover:bg-opacity-95 active:scale-[0.99] transition-all shadow-md shadow-brand-green/10 mt-2">
                Login
            </button>

            <div class="text-center pt-2 text-xs text-brand-brown/70">
                Belum punya akun catering? <a href="{{ route('register') }}" class="font-bold text-brand-brown hover:text-brand-green underline transition-colors">Daftar Sekarang</a>
            </div>
        </form>
    </div>
</body>
